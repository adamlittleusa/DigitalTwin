...
Add the import after `import requests`:
...
from tests.fakes import ExplodingStream, FakeBudget, FakeNotifier, ScriptedClient, chunk, text_stream, tool_stream, usage_chunk
...
Expected: every test fails because `TwinAgent` has no `run` and rejects the new keyword arguments.
...
def write_knowledge(root: Path) -> Path:
    ...
    write_md(root, "projects/digital-twin.md", project_meta("Digital twin"), "## What it is\n\nAn agent that represents Adam.")
    return root
```
...
```powershell
docker run --rm -d --name twin-api-check --env-file .env -p 8080:8080 twin-api
Start-Sleep -Seconds 5
curl.exe -s http://localhost:8080/v1/health
...
Expected: health JSON; a streamed reply mentioning Boston; a JSON log line with `"outcome": "ok"` and no message text. `--env-file .env` passes the secrets into the container without printing them; do not echo the file.
