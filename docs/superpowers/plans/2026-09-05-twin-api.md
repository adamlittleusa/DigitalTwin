...
- **Task 11.** Authored per the plan; the build could not be verified on the authoring machine (Docker Desktop backend crash on stale socket files under `%LOCALAPPDATA%\Docker\run`). Verify with Step 3 once Docker runs.
